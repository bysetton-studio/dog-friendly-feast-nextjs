...
export default function LogoutButton() {
  ...
  return (
    <>
      <button className="top-nav__link border-none cursor-pointer text-[#f28b82]!" onClick={() => setOpen(true)}>
        Log out
      </button>
      ...
      {open && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-100 pointer-events-auto" onClick={() => setOpen(false)}>
          <div className="bg-[rgba(30,30,30,0.98)] border border-white/10 rounded-[14px] pt-7 px-6 pb-6 w-full max-w-80 font-[Arial,sans-serif]" onClick={(e) => e.stopPropagation()}>
            <p className="text-[15px] text-[#e0e0e0] mt-0 mb-6 text-center">Are you sure you want to log out?</p>
            <div className="flex gap-2.5">
              <button className="flex-1 py-2.5 rounded-lg text-[14px] font-[Arial,sans-serif] cursor-pointer transition-[background] duration-150 bg-transparent border border-white/10 text-[#9aa0a6] hover:text-[#e0e0e0] hover:border-white/25" onClick={() => setOpen(false)}>
                Cancel
              </button>
